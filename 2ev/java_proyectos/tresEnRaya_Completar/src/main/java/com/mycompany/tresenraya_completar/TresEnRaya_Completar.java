...
package com.mycompany.tresenraya_completar;
...
/**
 *
 * @author Carlos Fernández Ayuso
 */
public class TresEnRaya_Completar{
    ...
    public static char coincidenciaDiagonal(char[][] matriz, char simboloDef){
        ...
        // Diagonal inversa
        coincidencia = true;
        simbolo = matriz[0][matriz.length - 1];
        if(simbolo != simboloDef){
            for(int i = 1; i < matriz.length; i++){
                if(simbolo != matriz[i][(matriz.length - 1) - i]){
                    coincidencia = false;
                    break;
                }
            }
